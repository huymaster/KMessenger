...
class UserDTO : BaseDTOImpl<UserEntity>() {
    companion object {
    ...
    override fun write(output: ObjectNode) {
        ...
        output.put(LAST_SEEN_FIELD, lastSeen)
        output.put(CREATED_AT_FIELD, createdAt)
    ...
    override fun read(input: JsonNode) {
        ...
        lastSeen = input.getOrDefault(LAST_SEEN_FIELD, 0L).asLong()
        createdAt = input.getOrDefault(CREATED_AT_FIELD, 0L).asLong()
